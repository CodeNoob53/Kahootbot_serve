...
app.get('/test-proxy', async (req, res) => {
  ...
  try {
    ...
    if (!httpsAgent) {
      console.log('Проксі-агент не ініціалізовано.')
      return res.status(500).json({
        success: false,
    ...
    try {
      ...
      const testResponse = await new Promise((resolve, reject) => {
        ...
        // Встановлюємо обробник таймауту
        proxyReq.on('timeout', () => {
          console.error('❌ Таймаут запиту до проксі')
          proxyReq.destroy()
      ...
      console.log(
        `✅ Відповідь від Kahoot (test-proxy): статус ${testResponse.statusCode}`
      )
      ...
      return res.json({
        ...
        testResponse: {
          ...
          isSuccess:
            testResponse.statusCode >= 200 && testResponse.statusCode < 500,
          timestamp: new Date().toISOString()
        }
      })
    } catch (requestError) {
      console.error(`❌ Помилка тестового запиту: ${requestError.message}`)
      return res.status(502).json({
        success: false,
        message: `Помилка з'єднання з Kahoot через проксі`,
        error: requestError.message
      })
    }
  } catch (error) {
    console.error(`❌ Загальна помилка тестування проксі: ${error.message}`)
    if (error.stack) {
    ...
    ws.on('message', message => {
      try {
        if (kahootWs.readyState === WebSocket.OPEN) {
          ...
          const logMsg =
            msgStr.length > logSize
              ? msgStr.substring(0, logSize) + '...'
              : msgStr
